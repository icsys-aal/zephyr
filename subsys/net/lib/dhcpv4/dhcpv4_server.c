...
static struct in_addr net_dhcpv4_server_find_free_address(struct dhcpv4_server_ctx *ctx, struct dhcpv4_client_id* client_id);
				   ...
				   uint8_t optlen)
{
	...
	for (unsigned i = 0; i < CONFIG_NET_DHCPV4_SERVER_ADDR_COUNT; ++i) {
		struct dhcpv4_addr_slot* slot = &ctx->addr_pool[i];
		if (slot->state == DHCPV4_SERVER_ADDR_FREE) {
			selected = slot;
		}
				  ...
				  uint8_t optlen)
{
	...
	if (ret == 0) {
		...
		{
			ret = address_validator_callback(ctx->iface, &client_id, &requested_ip, address_validator_callback_user_data);
			if(ret != 0)
			{
...
static struct in_addr net_dhcpv4_server_find_free_address(struct dhcpv4_server_ctx *ctx, struct dhcpv4_client_id *client_id)
{
	...
	for (unsigned int i = 0; i < CONFIG_NET_DHCPV4_SERVER_ADDR_COUNT; ++i)
	{
		...
		if (address_validator_callback &&
			address_validator_callback(ctx->iface, client_id, &address, address_validator_callback_user_data) != 0) {
			continue;
		}
		...
		for (unsigned int j = 0; j < CONFIG_NET_DHCPV4_SERVER_ADDR_COUNT; ++j)
		{
			...
			if (slot->state != DHCPV4_SERVER_ADDR_FREE && net_ipv4_addr_cmp(&address, &slot->addr)) {
				/* Address is not free */
				taken = true;
				break;
			}
